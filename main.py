...
def main():
    ...
    user_path = usr_input.get_usr_input(msg)  # Ask the user for the pat
    # Validate the path and get valid JSON files
    check_return = usr_input.check_path(user_path, [])  # Pass the user-provided path
...
def start_process(tup):
    ...
    """
    if not tup:  # Prevents processing if no valid files
        return
    ...
    for file_path in tup:
        if not file_path.endswith(".json") or file_path.endswith("_formatted.json"):
            continue  # Skip non-JSON or already formatted files
    ...
    print_output(num_files, num_emps)  # Keep this only if tests expect printed output
